...
SCREENWIDTH = 289
SCREENHEIGHT = 511
# For initializing a window or screen for display
...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():  # If user clicks on cross button, close the game
            ...
            # If the user presses space or up key start the game for them
                ...
                FPSCLOCK.tick(FPS)  # For controlling FPS
...
def getRandomPipe():
    ...
    pipe = [
        {'x': pipeX, 'y': -y1},  # -y1 because of upper
        {'x': pipeX, 'y': y2}    # it will reamain positive y2 because its lower
    ]
    return pipe


if __name__ == "__main__":  # This will be the main point from where the game will start
    pygame.init()  # Initialize all pygame's modules
    FPSCLOCK = pygame.time.Clock()  # Basically for controlling FPS
    pygame.display.set_caption('Flappy Nird by Dhruv')
